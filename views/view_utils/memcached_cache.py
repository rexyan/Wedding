...
__all__ = ['get_key', 'set_key', 'memorize', 'memcache_client']
...
def set_key(key, value, ex_time=settings.MEMCACHE_EXPIRE_TIME):
    """time为过期时间，以秒为单位"""
    return memcache_client.set(str(key), value, ex_time)
...
def memorize(func):
    ...
    def helper(*args, **kwargs):
        ...
        key = func.__name__ + hashlib.md5('#'.join(args_str)).hexdigest()
        # end make cache key
        remote_version = get_remote_obj_version(key)
        ret_obj = None
        if refresh:
